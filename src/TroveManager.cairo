...
pub mod TroveManager {
    ...
    use super::{
        ITroveManager, LatestBatchData, LatestTroveData, Liquidation,
        OnSetInterestBatchManagerParams, Operation, Redemption, RedemptionFeePaidToTrove,
        RewardSnapshots, Status, Trove, TroveChange, TroveOperation, TroveUpdated,
    };
    ...
    impl ITroveManagerImpl of ITroveManager<ContractState> {
        ...
        fn get_unbacked_portion_price_and_redeemability(
            ...
            let redeemable = self
                .liquity_base
                ._get_TCR(price.try_into().unwrap()) >= self
                .SCR
                .read()
                && self.shutdown_time.read() == 0;
            (unbacked_portion, price.try_into().unwrap(), redeemable)
        }
        ...
        fn batch_liquidate_troves(ref self: ContractState, trove_array: Span<u256>) {
            ...
            // TODO: We skip the below part for now.
        ...
        fn redeem_collateral(
            ...
            // Get the price to use for the redemption collateral calculations
            // TODO: We have to make specific function fetch_redemption_price here for if shutdown.
            // TODO: Oracle logic in general should be refactored, add validation checks
            let redemption_price = price_feed.fetch_redemption_price();
        ...
        fn urgent_redemption(
            ...
            // TODO: Implement urgent redemption
        }
        ...
        fn set_addresses(
            ...
            self.liquity_base.active_pool.write(active_pool);
            self.liquity_base.default_pool.write(default_pool);
            self.liquity_base.price_feed.write(price_feed);
            self.ETH.write(eth);
            ...
            self.bit_usd_token.write(bitusd);
    ...
    fn _apply_single_redemption(
        ...
        if (is_trove_in_batch) { // TODO: logic to add here
        } else {
            single_redemption
                .old_weighted_recorded_debt = single_redemption
                .trove
